...
class SteeredDiffusionSampler(DiffusionSampler):
    ...
    def denoise_step(self, b_t, t, t_next, eta, gamma, control_dict_T, control_dict_P, **prior_kwargs):
        ...
        b_t_next = self.DP.reverse_step(b_t, t, t_next, self.BB, self.pred_type, eta, self.prior, **prior_kwargs)
        for channel, channel_control in control_dict_T.items():
            # logger.debug(f"Setting channel {channel} to {channel_control}")
            b_t_next[:, channel] = (1 - gamma) * b_t_next[:, channel] + gamma * channel_control #control_dict_P has not been used here 
        return b_t_next
    ...
    def sample_batch(self, eta=1, gamma=0, batch_size=1000, temperature=1, pressure =1, **kwargs):
        """
        Sample a batch of data.
