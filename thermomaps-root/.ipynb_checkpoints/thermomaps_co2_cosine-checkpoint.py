...
highP = np.cos(highP[:, :, :, :]/(180/np.pi))
lowP = np.cos(lowP[:, :, :, :]/(180/np.pi))
...
pressure = np.array([-2.0, 2.0])
pressure_list = np.ones(int(lowP.shape[0]*2))
temperature_list = 5* np.ones(int(lowP.shape[0]*2))

for i in range(2):
    pressure_list[int(lowP.shape[0])*i:int(lowP.shape[0])*(i+1)] = pressure[i]
...
high_samples = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=1.5 , temperature=1)
low_samples = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=-1.5 , temperature=1)


def convert_data_visual(data):
    if not isinstance(data, np.ndarray):
        data = data.numpy()
    samples = np.concatenate(np.concatenate(data[:,:12,:,:]))
...
x_p1 = np.concatenate(np.concatenate(low_samples[:,:12,:,:]))
x_p1 = np.concatenate(x_p1)
x_p12 = np.concatenate(np.concatenate(high_samples[:,:12,:,:]))
...
for i in temp: 
    locals()['high_samples_t_{0}'.format(i)] = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=1.5 , temperature=i)
    locals()['low_samples_t_{0}'.format(i)] = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=-1.5 , temperature=i)
...
for i in pres: 
    locals()['high_samples_p_{0}'.format(i)] = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=i/10 , temperature=2)
    locals()['low_samples_p_{0}'.format(i)] = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=-i/10 , temperature=2)
...
highP_denisty = np.cos(highP[400::20, :, :, :12]/(180/np.pi))
highP_denisty = highP_denisty.transpose(0,3,1,2)
lowP_denisty = np.cos(lowP[400::20, :, :, :12]/(180/np.pi))
...
for i,sample in enumerate(highP_samp):
    sns.kdeplot(convert_data_visual(sample),label='P=%i'%pres[i], fill=True)#, color="blue")
plt.legend()
plt.xlabel('angle (not scaled)')
...
for i,sample in enumerate(high_samp):
    sns.kdeplot(convert_data_visual(sample),label='T=%i'%temp[i], fill=True)#, color="blue")
plt.legend()
plt.xlabel('angle (not scaled)')
...
for j in range(12):
    # for i in range(1):
    plt.subplot(4,6,j+1)
    plt.imshow(high_samples[0,j,:,:],vmin=0,vmax=1)
...
for j in range(12):
    ...
    plt.imshow(low_samples[0,j,:,:],vmin=0,vmax=1)
...
high_samples_p5t5 = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=1.5 , temperature=2)
low_samples_p5t5 = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=-1.5 , temperature=2)
...
plt.subplot(1,2,1)
sns.kdeplot(convert_data_visual(highP_denisty),label='ref', fill=False, color="blue")
sns.kdeplot(convert_data_visual(high_samples_p5t5),label='high p5 t5', fill=True)#, color="blue")
plt.legend()
plt.xlabel('angle (not scaled)')

plt.subplot(1,2,2)
sns.kdeplot(convert_data_visual(lowP_denisty),label='ref', fill=False, color="red")
sns.kdeplot(convert_data_visual(low_samples_p5t5),label='low p5t5', fill=True)#, color="blue")
plt.legend()
plt.xlabel('angle (not scaled)')
plt.savefig('plots/temperature_p5t5_dependent.pdf')

plt.figure(figsize=(5,4.5))
# plt.subplot(1,2,1)
sns.kdeplot(convert_data_visual(highP_denisty),label='ref H', fill=False, color="blue")
sns.kdeplot(convert_data_visual(lowP_denisty),label='ref L', fill=False, color="red")

sns.kdeplot(convert_data_visual(high_samples_p5t5),label='high p=1.5 t=5', fill=True)#, color="blue")
sns.kdeplot(convert_data_visual(low_samples_p5t5),label='low p=-1.5 t=5', fill=True)#, color="blue")
# plt.subplot(1,2,2)
plt.legend()
plt.xlabel('angle (not scaled)')
plt.savefig('plots/temperature_p5t5_together.pdf')


# #######
# high_samples_p5t3 = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=1.5 , temperature=2)
# low_samples_p5t3 = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=-1.5 , temperature=2)
...
# plt.subplot(1,2,1)
# sns.kdeplot(convert_data_visual(highP_denisty),label='ref', fill=False, color="blue")
# sns.kdeplot(convert_data_visual(high_samples_p5t3),label='high p5 t3', fill=True)#, color="blue")
# plt.legend()
# plt.xlabel('angle (not scaled)')

# plt.subplot(1,2,2)
# sns.kdeplot(convert_data_visual(lowP_denisty),label='ref', fill=False, color="red")
# sns.kdeplot(convert_data_visual(low_samples_p5t3),label='low p5 t3', fill=True)#, color="blue")
# plt.legend()
# plt.xlabel('angle (not scaled)')
# plt.savefig('plots/temperature_p5t3_dependent.pdf')
...
# sns.kdeplot(convert_data_visual(high_samples_p5t3),label='high p=1.5 t=3', fill=True)#, color="blue")
# sns.kdeplot(convert_data_visual(low_samples_p5t3),label='low p=-1.5 t=3', fill=True)#, color="blue")
# # plt.subplot(1,2,2)
# plt.legend()
# plt.xlabel('angle (not scaled)')
# plt.savefig('plots/temperature_p5t3_together.pdf')


# ############

# num_samp=1
# high_samples_p05t3 = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=0.5 , temperature=2)
# low_samples_p05t3 = sampler.sample_loop(num_samples=num_samp, batch_size=32, pressure=-0.5 , temperature=2)
...
# plt.subplot(1,2,1)
# sns.kdeplot(convert_data_visual(highP_denisty),label='ref', fill=False, color="blue")
# sns.kdeplot(convert_data_visual(high_samples_p05t3),label='high p05 t3', fill=True)#, color="blue")
# plt.legend()
# plt.xlabel('angle (not scaled)')

# plt.subplot(1,2,2)
# sns.kdeplot(convert_data_visual(lowP_denisty),label='ref', fill=False, color="red")
# sns.kdeplot(convert_data_visual(low_samples_p05t3),label='low p05 t3', fill=True)#, color="blue")
# plt.legend()
# plt.xlabel('angle (not scaled)')
# plt.savefig('plots/temperature_p05t5_dependent.pdf')
...
# sns.kdeplot(convert_data_visual(high_samples_p05t3),label='high p=1.5 t=3', fill=True)#, color="blue")
# sns.kdeplot(convert_data_visual(low_samples_p05t3),label='low p=-1.5 t=3', fill=True)#, color="blue")
# # plt.subplot(1,2,2)
# plt.legend()
# plt.xlabel('angle (not scaled)')
# plt.savefig('plots/temperature_p05t5_together.pdf')
